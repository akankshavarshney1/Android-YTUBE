 ...
 */
package free.rm.Video.businessobjects.YouTube.newpipe;
...
public class NewPipeService {
    ...
    public synchronized static NewPipeService get() {
        if (instance == null) {
            getHttpDownloader();
            instance = new NewPipeService(ServiceList.YouTube, VideoMain.getSettings());
        }
        return instance;
    }
    ...
    public static OkHttpDownloader getHttpDownloader() {
        ...
        if (NewPipe.getDownloader() == null) {
            NewPipe.init(downloader, Localization.DEFAULT, toContentCountry(VideoMain.getSettings().getPreferredContentCountry()));
        }
        return downloader;
    }
    ...
    public static boolean isPreferred() {
        return VideoMain.getPreferenceManager().getBoolean(VideoMain.getStr(R.string.pref_use_default_newpipe_backend), true);
    }
}
