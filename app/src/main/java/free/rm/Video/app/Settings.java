...
import free.rm.Video.gui.fragments.MainFragment;
import free.rm.Video.gui.fragments.SubscriptionsFeedFragment;

/**
 * Type safe wrapper to access the various preferences.
 */
public class Settings {
    private final VideoMain app;
    ...
    public Set<String> getSponsorblockCategories() {
        String[] defaultFilterList = VideoMain.getStringArray(R.array.sponsorblock_filtering_list_default_values);
        Set<String> filterList = VideoMain.getPreferenceManager().getStringSet("pref_key_sponsorblock_category_list", new HashSet<String>(Arrays.asList(defaultFilterList)));

        return filterList;
    }

    public boolean isSponsorblockEnabled() {
        return VideoMain.getPreferenceManager().getBoolean("pref_key_enable_sponsorblock", false);
    }
    ...
    public StreamSelectionPolicy getDesiredVideoResolution(boolean forDownload, boolean onMetered) {
        ...
        String maxKey = VideoMain.getStr(forDownload ? R.string.pref_key_video_download_maximum_resolution : R.string.pref_key_maximum_res);
        String maxResIdValue = prefs.getString(maxKey, Integer.toString(VideoResolution.DEFAULT_VIDEO_RES_ID));

        String minKey = VideoMain.getStr(forDownload ? R.string.pref_key_video_download_minimum_resolution : R.string.pref_key_minimum_res);
        String minResIdValue = prefs.getString(minKey, null);

        String qualityKey = VideoMain.getStr(forDownload ? R.string.pref_key_video_quality_for_downloads : R.string.pref_key_video_quality);
        ...
        if (onMetered) {
            ...
            maxResIdValue = prefs.getString(VideoMain.getStr(R.string.pref_key_maximum_res_mobile), maxResIdValue);
            minResIdValue = prefs.getString(VideoMain.getStr(R.string.pref_key_minimum_res_mobile), minResIdValue);
            qualityValue = prefs.getString(VideoMain.getStr(R.string.pref_key_video_quality_on_mobile), qualityValue);
        }
        ...
        boolean useNewFormats = prefs.getBoolean(VideoMain.getStr(R.string.pref_key_use_newer_formats), false);
    ...
    public StreamSelectionPolicy getDesiredVideoResolution(boolean forDownload) {
        return getDesiredVideoResolution(forDownload, VideoMain.isActiveNetworkMetered());
    }

    public boolean isDisableSearchHistory() {
        return getSharedPreferences().getBoolean(VideoMain.getStr(R.string.pref_key_disable_search_history), false);
    }
    ...
    public Long getFeedsLastUpdateTime() {
        long l = getSharedPreferences().getLong(VideoMain.KEY_SUBSCRIPTIONS_LAST_UPDATED, -1);
        return (l != -1)  ?  l  :  null;
    }
    ...
    public void updateFeedsLastUpdateTime(Long dateTimeInMs) {
        setPreference(VideoMain.KEY_SUBSCRIPTIONS_LAST_UPDATED, dateTimeInMs != null ? dateTimeInMs : -1);
    }
    ...
    private String getPreference(@StringRes int resId, String defaultValue) {
        return getSharedPreferences().getString(VideoMain.getStr(resId), defaultValue);
    }

    private boolean getPreference(@StringRes int resId, boolean defaultValue) {
        return getSharedPreferences().getBoolean(VideoMain.getStr(resId), defaultValue);
    }
    ...
    private Set<String> getPreference(@StringRes int resId, Set<String> defaultValue) {
        return getSharedPreferences().getStringSet(VideoMain.getStr(resId), defaultValue);
    }
