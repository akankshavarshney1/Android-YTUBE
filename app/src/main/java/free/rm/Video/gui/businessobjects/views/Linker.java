...
package free.rm.Video.gui.businessobjects.views;
	...
	static class LinkListener implements TouchableMovementMethod.URLSpanClickListener {
		...
		public void onClick(URLSpan span, boolean longClick) {
			if (longClick) {
				...
				VideoMain.openUrl(ctx, span.getURL(), true);
			}
		}
		...
		private void longClick(String clickedText) {
			...
			ContentId content = VideoMain.parseUrl(ctx, clickedText, false);
			...
			new AlertDialog.Builder(ctx)
					...
					.setItems(items.toArray(new CharSequence[0]),
							(dialog, which) -> {
								switch (which) {
									case 0:
										VideoMain.viewInBrowser(clickedText, ctx);
										break;
									case 1:
										VideoMain.copyUrl(ctx, "URL", clickedText);
										break;
									case 2:
