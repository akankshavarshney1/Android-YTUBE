...
import free.rm.Video.databinding.VideoCellBinding;
...
import free.rm.Video.gui.businessobjects.YouTubePlayer;
...
public class GridViewHolder extends RecyclerView.ViewHolder implements Serializable {
	...
	private void onOptionsButtonClick(final View view, YouTubeChannel channel) {
		...
		popupMenu.setOnMenuItemClickListener(item -> {
			switch (item.getItemId()) {
				case R.id.share:
					VideoMain.shareUrl(context, channel.getChannelUrl());
					return true;
				case R.id.copyurl:
					VideoMain.copyUrl(context, "Channel URL", channel.getChannelUrl());
					return true;
				...
				case R.id.open_channel:
					VideoMain.launchChannel(channel.getId(), context);
					return true;
	...
	private void onOptionsButtonClick(final View view, YouTubeVideo youTubeVideo) {
		...
		if(!VideoMain.getPreferenceManager().getBoolean(context.getString(R.string.pref_key_disable_playback_status), false)) {
		...
		boolean online = VideoMain.isConnected(view.getContext());
		...
		if(VideoMain.getPreferenceManager().getBoolean(context.getString(R.string.pref_key_enable_video_blocker), true)) {
