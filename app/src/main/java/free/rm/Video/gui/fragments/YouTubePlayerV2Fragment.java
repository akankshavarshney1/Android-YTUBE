...
import free.rm.Video.app.Settings;
import free.rm.Video.app.VideoMain;
import free.rm.Video.app.StreamSelectionPolicy;
import free.rm.Video.app.Utils;
import free.rm.Video.app.enums.Policy;
import free.rm.Video.businessobjects.Logger;
import free.rm.Video.businessobjects.Sponsorblock.SBSegment;
import free.rm.Video.businessobjects.Sponsorblock.SBTasks;
import free.rm.Video.businessobjects.Sponsorblock.SBTimeBarView;
import free.rm.Video.businessobjects.Sponsorblock.SBVideoInfo;
...
import free.rm.Video.businessobjects.YouTube.YouTubeTasks;
import free.rm.Video.businessobjects.YouTube.newpipe.ContentId;
import free.rm.Video.businessobjects.db.DatabaseTasks;
import free.rm.Video.businessobjects.db.DownloadedVideosDb;
import free.rm.Video.businessobjects.db.PlaybackStatusDb;
import free.rm.Video.businessobjects.interfaces.GetDesiredStreamListener;
import free.rm.Video.businessobjects.interfaces.PlaybackStateListener;
import free.rm.Video.businessobjects.interfaces.YouTubePlayerActivityListener;
import free.rm.Video.businessobjects.interfaces.YouTubePlayerFragmentInterface;
import free.rm.Video.databinding.FragmentYoutubePlayerV2Binding;
import free.rm.Video.databinding.VideoDescriptionBinding;
import free.rm.Video.gui.activities.ThumbnailViewerActivity;
import free.rm.Video.gui.businessobjects.DatasourceBuilder;
import free.rm.Video.gui.businessobjects.MobileNetworkWarningDialog;
import free.rm.Video.gui.businessobjects.PlaybackSpeedController;
import free.rm.Video.gui.businessobjects.PlayerViewGestureDetector;
import free.rm.Video.gui.businessobjects.ResumeVideoTask;
import free.rm.Video.gui.businessobjects.SkyTubeMaterialDialog;
import free.rm.Video.gui.businessobjects.adapters.CommentsAdapter;
import free.rm.Video.gui.businessobjects.fragments.ImmersiveModeFragment;
import free.rm.Video.gui.businessobjects.views.Linker;
...
public class YouTubePlayerV2Fragment extends ImmersiveModeFragment implements YouTubePlayerFragmentInterface {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        playerViewGestureHandler = new PlayerViewGestureHandler(VideoMain.getSettings());
...
//		});
        ...
        if (youTubeVideo == null) {
            ...
            if (bundle != null && bundle.getSerializable(YOUTUBE_VIDEO_OBJ) != null) {
                ...
                final ContentId contentId = VideoMain.getUrlFromIntent(requireContext(), intent);
    ...
    private void initViews() {
        ...
        videoDescriptionBinding.videoDescChannelThumbnailImageView.setOnClickListener(v -> {
            if (youTubeChannel != null) {
                VideoMain.launchChannel(youTubeChannel, getActivity());
            }
        });
        ...
        //set playback speed
        float playbackSpeed = VideoMain.getSettings().getDefaultPlaybackSpeed();
    ...
    private void setupInfoDisplay(YouTubeVideo video) {
        ...
        if (VideoMain.getSettings().isSponsorblockEnabled()) {
            initSponsorBlock();
        }
    }
                            ...
                            if (decision == Policy.ALLOW) {
                                ...
                                if (!youTubeVideo.isLiveStream()) {
                                    ...
                                    if (downloadStatus.getUri() != null) {
                                        ...
                                        if (VideoMain.getSettings().isSponsorblockEnabled() && sponsorBlockVideoInfo == null) {
                                                ...
                                                YouTubeTasks.getDesiredStream(youTubeVideo,
                                                        new GetDesiredStreamListener() {
                                                            ...
                                                            public void onGetDesiredStream(StreamInfo desiredStream, YouTubeVideo video) {
                                                                ...
                                                                if (isVisible()) {
                                                                    StreamSelectionPolicy selectionPolicy = VideoMain.getSettings().getDesiredVideoResolution(false);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.disable_gestures:
                ...
                VideoMain.getSettings().setDisableGestures(disableGestures);
    ...
    class PlayerViewGestureHandler extends PlayerViewGestureDetector {
        ...
        public void onCommentsGesture() {
            if (VideoMain.isConnected(requireContext())) {
    ...
    private static class VideoBrightness {
        ...
        private static final String BRIGHTNESS_LEVEL_PREF = VideoMain.getStr(R.string.pref_key_brightness_level);
        ...
        private void saveBrightnessToPreference() {
            SharedPreferences.Editor editor = VideoMain.getPreferenceManager().edit();
        ...
        private void loadBrightnessFromPreference() {
            final float brightnessPref = VideoMain.getPreferenceManager().getFloat(BRIGHTNESS_LEVEL_PREF, 1);
