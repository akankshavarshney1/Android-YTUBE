...
import free.rm.Video.businessobjects.FeedUpdaterService;
import free.rm.Video.businessobjects.VideoCategory;
import free.rm.Video.businessobjects.YouTube.Tasks.GetSubscriptionVideosTaskListener;
import free.rm.Video.databinding.FragmentSubsFeedBinding;
import free.rm.Video.gui.businessobjects.SubscriptionsBackupsManager;
...
public class SubscriptionsFeedFragment extends VideosGridFragment implements GetSubscriptionVideosTaskListener {
	...
	public void onResume() {
		...
		Settings settings = VideoMain.getSettings();
	...
	private synchronized void startRefreshTask(boolean isShowFetchingVideosDialog, boolean forcedFullRefresh) {
		...
		if (forcedFullRefresh && VideoMain.isConnected(requireContext())) {
			if (isShowFetchingVideosDialog) {
				showFetchingVideosDialog();
			}
	...
	public String getFragmentName() {
		return VideoMain.getStr(R.string.feed);
	}
