...
import free.rm.Video.databinding.FragmentYoutubePlayerV1Binding;
import free.rm.Video.databinding.VideoDescriptionBinding;
import free.rm.Video.gui.activities.ThumbnailViewerActivity;
import free.rm.Video.gui.businessobjects.MediaControllerEx;
import free.rm.Video.gui.businessobjects.MobileNetworkWarningDialog;
import free.rm.Video.gui.businessobjects.OnSwipeTouchListener;
...
public class YouTubePlayerV1Fragment extends ImmersiveModeFragment implements MediaPlayer.OnPreparedListener, YouTubePlayerFragmentInterface {
	...
	public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		if (youTubeVideo == null) {
			...
			if (bundle != null  &&  bundle.getSerializable(YOUTUBE_VIDEO_OBJ) != null) {
				...
				final ContentId contentId = VideoMain.getUrlFromIntent(requireContext(), requireActivity().getIntent());
	...
	private void initViews() {
		...
		fragmentBinding.voidView.setOnTouchListener(new OnSwipeTouchListener(getActivity()) {
			...
			private final boolean disableGestures = VideoMain.getPreferenceManager().getBoolean(VideoMain.getStr(R.string.pref_key_disable_screen_gestures), false);
									...
									YouTubeTasks.getDesiredStream(youTubeVideo,
										new GetDesiredStreamListener() {
											...
											public void onGetDesiredStream(StreamInfo desiredStream, YouTubeVideo video) {
												...
												if (isVisible()) {
													StreamSelectionPolicy selectionPolicy = VideoMain.getSettings().getDesiredVideoResolution(false).withAllowVideoOnly(false);
