...
public class YouTubePlayer {
	...
	private static void launchOnChromecast(final YouTubeVideo youTubeVideo, final Context context) {
		if(connectingToChromecast) {
			...
			if (context instanceof ChromecastListener) {
				...
				if(VideoMain.getSettings().isPlaybackStatusEnabled() && status.getPosition() > 0) {
	...
	private static boolean useOfficialYouTubePlayer(Context context) {
		...
		String              str = sharedPref.getString(VideoMain.getStr(R.string.pref_key_choose_player), VideoMain.getStr(R.string.pref_default_player_value));

		return  (str.equals(VideoMain.getStr(R.string.pref_official_player_value)));
	}
